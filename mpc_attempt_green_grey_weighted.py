...
def build_and_solve_window(T_stack_init, P_stack_init, y_off_prev,
    ...
    # def h2_min_rule(m):
    #      return (sum(m.h2_green[t] for t in range(1, CONTROL_HORIZON+1))
    #         + sum(m.h2_shortfall[t] for t in range(1, CONTROL_HORIZON+1))
    #         >= remaining_needed)
    ...
    print(f"  y_off[1..6]: {[round(pyo.value(model.y_off[c])) for c in range(1,7)]}")
    print(f"  y_cold_start[1..6]: {[round(pyo.value(model.y_cold_start[c])) for c in range(1,7)]}")
    print(f"  wind[1..6]: {[round(wind_win[c]) for c in range(1,7)]}")
    print(f"  price[1..6]: {[round(price_win[c],1) for c in range(1,7)]}")
    return model
...
while k < TOTAL_REAL_STEPS:
    ...
    committed_h2_this_window = sum(pyo.value(model.h2_green[t]) for t in range(1, CONTROL_HORIZON+1))
    k += CONTROL_HORIZON
    

mpc_results = pd.DataFrame(committed)
mpc_results.to_csv("mpc_results_green_grey_weighted.csv", index=False)
