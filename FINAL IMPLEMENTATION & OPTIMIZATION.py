...
class ElectrolyzerSimulation:
    ...
    def run_simulation(self):
        ...
        while current_time <= end_time:
            ...
            #             cooling_system_active = True
            #             # Steady-state cooling power required to maintain temperature once at target
            #             q_cooling_steady = q_gen_stack - q_loss_stack
                        
            #             # Maximum heat removal rate your system is physically capable of delivering
            #             q_cooling_max = q_limit_physical
                        
            #             # Total excess thermal energy trapped inside the stack structure (Joules)
            #             excess_energy_joules = PEM.C_TOTAL_J_K * temperature_error
                        
            #             # Net heat removal margin available above the active load baseline
            #             net_removal_capacity = q_cooling_max - q_cooling_steady
            #             if net_removal_capacity > 0:
            #                 # Calculate exactly how many seconds it takes to bring the stack back down to target_T
            #                 time_to_cool_seconds = excess_energy_joules / net_removal_capacity
                            
            #                 if time_to_cool_seconds < dt_seconds:
            #                     # --- PROVEN INTAKE TIME-WEIGHTED CLIPPING MODE ---
            #                     # The system hits the target mid-step!
            #                     t_max = time_to_cool_seconds
            #                     t_steady = dt_seconds - t_max
                                
            #                     # Combined integrated average cooling across the 10-minute block
            ...
            #                 else:
            #                     # Overheating is too severe to overcome in 10 mins; run max cooling full-time
            ...
            #             else:
            #                 # Steady state generation is already surpassing heat exchanger limits
            #                 q_cooling_stack = q_cooling_max
                            
            #                 # Calculate the unpreventable surplus energy building up per second (Watts)
            #                 unpreventable_heat_flux = q_cooling_steady - q_cooling_max
            #                 # Translate that directly into an exact temperature spike rate
            ...
            #         cooling_system_active = False
            #         q_cooling_stack= 0
            #         print(f"[{current_time}] ⚠️ BOUNDARY EXCEEDED: T = {self.current_T:.2f} K out of operational range.")
                    # YOUR EXACT NESTED RESOLUTION FUNCTION
            elif state == "OPERATIONAL":
                cold_start_minutes_step = 0.0
