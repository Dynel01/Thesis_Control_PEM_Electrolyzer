...
def build_and_solve_window(T_stack_init, P_stack_init, y_off_prev,
                            price_win, wind_win, epsilon=0.02,
                            mip_gap=0.005, verbose=True):
    """
    ...
    def on_temp_rule(m, t):
        return T_prev_of(m, t) >= (T_limit + 1) - M_temp * (1 - m.y_on[t])
    ...
    def power_limit_rule(m, t):
        return m.P_stack[t] + P_aux * (1 - m.y_off[t]) <= wind_win[t]
    model.power_limit = pyo.Constraint(model.T, rule=power_limit_rule)

    def grid_def_rule(m, t):
        return m.P_grid[t] == wind_win[t] - m.P_stack[t] - P_aux * (1 - m.y_off[t])
    ...
    # Stage 1 is a much simpler problem (pure H2 sum only) -- give it a tight
    # gap so the timing preference from `discount` can't be lost inside a
    # loose tolerance. 0.999^t only differed by 0.1% step-to-step, which was
    # smaller than the 0.5% gap we use for Stage 2 -- Gurobi had no reason to
    # find the "start now" schedule when "start later" was already within
    # its allowed tolerance. A stronger discount (0.97) plus a tight Stage 1
    # gap fixes both sides of that problem.
    solver.options['MIPGap'] = 0.0005
    solver.solve(model, tee=verbose)

    # Record STAGE 1's actual per-step schedule -- not just the total. This
    ...
    # indefinitely (since every freshly-rebuilt window looked the same,
    # "delay one more step" never actually cost anything). Per-step floors
    # close that loophole: Stage 2 must track Stage 1's timing, not just
    # its grand total.
    # -----------------------------------------------------
    ...
    def objective_rule(m):
        revenue_h2 = sum(discount[t] * pi_h2 * m.m_h2[t] for t in m.T)
        grid_rev = sum(discount[t] * (price_win[t] / 1e3) * (1 / 6) * m.P_grid[t] for t in m.T)
        degradation = sum(discount[t] * m.degradation_cost[t] for t in m.T)
        heat_rev = sum(
            discount[t] * (
...
# 4. MPC ROLLING LOOP
# =========================================================
WINDOW_LEN = 144          # 24 hours at 10-min resolution
NUM_WINDOWS = 200          # <-- start small to measure real per-window solve
                          #     time before committing to a full run
...
# Real time 0 is the true, known simulation start -- trivially OFF, no
# solve needed for it, matching the original script's anchor exactly.
committed = [{
    ...
    'wind': float(df.loc[0, 'wtc_ActPower_mean']),
    'price': float(df.loc[0, 'SpotPrice_DK1']) * 3,
    'window_H2_max': None, 'active_mode': None,
}]

for k in range(NUM_WINDOWS):
    price_win = {t: float(df.loc[k + t, 'SpotPrice_DK1']) * 3 for t in range(WINDOW_LEN)}
    wind_win = {t: float(df.loc[k + t, 'wtc_ActPower_mean']) for t in range(WINDOW_LEN)}

    model, H2_max = build_and_solve_window(
        T_stack_current, P_stack_current, y_off_current,
        price_win, wind_win, epsilon=EPSILON, verbose=False
    )

    # Commit t=1 -- the first genuinely free decision in this window,
    # corresponding to REAL time k+1. t=0 is just this window's fixed,
    # already-known anchor (== whatever was committed as real time k).
    active_mode = None
    for i in model.I:
        if pyo.value(model.x[1, i]) > 0.5:
            active_mode = i
            break

    committed.append({
        'time': k + 1,
        'P_stack': pyo.value(model.P_stack[1]),
        'T_stack': pyo.value(model.T_stack[1]),
        'H2': pyo.value(model.m_h2[1]),
        'Q_delivered': pyo.value(model.Q_delivered[1]),
        'W_pump': pyo.value(model.W_pump[1]),
        'degradation_cost': pyo.value(model.degradation_cost[1]),
        'y_on': pyo.value(model.y_on[1]),
        'y_off': pyo.value(model.y_off[1]),
        'y_standby': pyo.value(model.y_standby[1]),
        'y_cold_start': pyo.value(model.y_cold_start[1]),
        'wind': wind_win[1],
        'price': price_win[1],
        'window_H2_max': H2_max,
        'active_mode': active_mode,
    })

    # Real state carried into the next window -- t=1's outcome becomes
    # the next window's t=0 anchor.
    T_stack_current = pyo.value(model.T_stack[1])
    P_stack_current = pyo.value(model.P_stack[1])
    y_off_current = pyo.value(model.y_off[1])

    print(f"[window {k}] committed (real t={k+1}) P={committed[-1]['P_stack']:.0f} kW, "
          f"T={committed[-1]['T_stack']:.2f} K, H2={committed[-1]['H2']:.2f} kg")

mpc_results = pd.DataFrame(committed)
mpc_results.to_csv("mpc_results.csv", index=False)
print(f"\nDone. {NUM_WINDOWS} windows committed, results in mpc_results.csv")
