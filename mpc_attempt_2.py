...
BASE_IDX = 1830          # 2014-01-11 12:00 -- start of the demonstration period
TOTAL_REAL_STEPS = 216    # 1.5 days centered on the known wind dip
CONTROL_HORIZON = 6       # commit 6 steps (1 hour) before re-solving, cuts
                          # total solve count ~6x vs re-solving every step
...
while k < TOTAL_REAL_STEPS:
    ...
    # Commit CONTROL_HORIZON steps (1..CONTROL_HORIZON), not just t=1 --
    # this is what cuts total solve count roughly 6x for the same coverage.
    ...
    # not step 1 -- that's the real state at the moment we're about to
    # re-solve next.
...
mpc_results.to_csv("mpc_results_2.csv", index=False)
